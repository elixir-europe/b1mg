<?php
   $title = "Resources from the Beyond 1 Million Genomes (B1MG) project";
   $description = "Publications, media and deliverables from the B1MG project.";
   $section = "resources";
   $page = "resources";
   include $_SERVER['DOCUMENT_ROOT']."/includes/header.inc";
?>
<section class="hero is-primary">
    <div class="hero-body py-5">
        <div class="container">
            <h1 class="title">
                Resources
            </h1>
        </div>
    </div>
</section>
<section class="section pt-55">
    <div class="container content has-text-grey-darker">
        <p>The B1MG project started in June 2020. As the project progresses we will be adding publications, publicity materials and the project deliverables to this section.</p>
        <h2>Publications</h2>
        <div class="columns is-mobile">
            <div class="column has-text-centered mb-5 is-one-quarter">
                <h5 class="pt-4">1+MG HEOR Summary Brief</h5>
                <figure class = "m-0">
                    <a href="/images/pdf/1+MG-HEOR-Summary-paper.pdf"><img class="border-grey" src="/images/b1mg-publication-1.png" alt="Screenshot of 1+MG HEOR Summary brief" /></a>
                </figure>
            </div>
            <div class="column has-text-centered mb-5 is-one-third">
                <h5 class="pt-4">Genomics Policy Brief in Healthcare</h5>
                <figure class = "m-0">
                    <a href="/images/pdf/Policy_Brief_Genomics_in_Healthcare_2022.pdf"><img class="border-grey" src="/images/genome_policy_brief.jpg" alt="Screenshot of 1+MG HEOR Summary brief" /></a>
                </figure>
            </div>
        </div>
        <!--<div class="card px-3 py-3 mt-55 grey announcement has-text-weight-bold"> The Stakeholder Portal is now live - see the <a href="https://b1mg-project.eu/news-events/index.php#stakeholder-portal">news story</a> or go to the <a href="https://docs.google.com/forms/d/e/1FAIpQLSfDRtFrsYIIkOjz0343Rub4F-74foqt8MZJFXsFg1iVyMGc2w/viewform">registration form</a>.</div>-->
        <h2 class="mt-0">Videos</h2>
<h3 class="mt-0">Country visits (2021)</h3>
<p>In the first half of 2021 the B1MG project conducted virtual country visits, to find out how different countries in the project managed and used genomics data in healthcare. There were three country visits: to the UK, Estonia and Finland. Each visit also included <strong>flash talks</strong> from other countries: Belgium, Bulgaria, Denmark, Hungary, Latvia, Lithuania, Portugal, Spain and Sweden.</p>
<p>The country visits were organised by <a href="/work-packages/wp5">Work Package 5</a> of the project. The goal was to exchange ideas and foster creative solutions for using genomic data in healthcare. These ideas will help the Work Package develop a roadmap for the adoption of genomic data in healthcare across Europe.</p>
<div class="columns mt-5 mb-0">
  <div class="column has-text-centered bordered mb-5">
    <h3>UK visit</h3>
    <a href="https://youtube.com/playlist?list=PLweO8RYcVPDOWDmFxRHdEGlNsMEubMK5k"><img src="/images/videos/country-visit-uk.png" alt="Screenshot of UK visit video" /></a>
    <p>(March 2021, 12 videos)</p>
  </div>
  <div class="column has-text-centered bordered mb-5">
    <h3>Estonia visit</h3>
    <a href="https://www.youtube.com/playlist?list=PLweO8RYcVPDMTY_Xi0nLSOtD67jeydJJ7"><img src="/images/videos/country-visit-estonia.png" alt="Screenshot of Estonia visit video" /></a>
    <p>(May 2021, 10 videos)</p>
  </div>
  <div class="column has-text-centered bordered mb-5">
    <h3>Finland visit</h3>
    <a href="https://www.youtube.com/playlist?list=PLweO8RYcVPDM5ydSVQuQO7WBpkZm8dycy"><img src="/images/videos/country-visit-finland.png" alt="Screenshot of Finland visit video" /></a>
    <p>(June 2021, 10 videos)</p>
  </div>
  <div class="column has-text-centered bordered mb-5">
    <h3>Flash talks</h3>
    <a href="https://www.youtube.com/playlist?list=PLweO8RYcVPDM9Bo_safKw9bQDWueEx6ZA"><img src="/images/videos/country-visit-members.png" alt="Screenshot of the flash talks video" /></a>
    <p>(9 videos)</p>
  </div>
</div>


        <h2 class="mt-55">Deliverables</h2>
        <p>The project deliverables are all reports, and they will all be public except D6.2 (Project Data Management Plan). This is a report for project members only.</p>
        <p> Links to the deliverables will be added as they are completed. You can find more details about the deliverables and the tasks that produced them in the <a href="/work-packages">Work Packages section</a>.</p>
      <!--  <table class="table is-bordered mt-5">
          <thead>
          <tr class="is-selected is-size-5">
            <th class="percent-15">Due date</th>
            <th>Name</th>
            <th>Number</th>
            <th>WP</th>
          </tr>
        </thead>
        <tbody>
          <tr>
            <td>October 2020</td>
            <td>Document describing the operational organisation and processes for the Stakeholder Coordination Group (SCG)</td>
            <td>D1.1</td>
            <td>WP1</td>
          </tr>
          <tr>
            <td>May 2021</td>
            <td><a href="Generation of a stakeholder portal within the project webpage containing the information resulting from the activities of the SCG</td>
            <td>D1.2</td>
            <td>WP1</td>
          </tr>
          <tr>
            <td>May 2021</td>
            <td>First yearly report on SCG recommendations to the 1+MG national mirror groups to facilitate genome/ clinical data integration, specified to the 1+MG Use Cases.</td>
            <td>D1.3</td>
            <td>WP1</td>
          </tr>
          <tr>
            <td>May 2022</td>
            <td>Second yearly report on SCG recommendations to the 1+MG national mirror groups to facilitate genome/ clinical data integration, specified to the 1+MG Use Cases.</td>
            <td>D1.4</td>
            <td>WP1</td>
          </tr>
          <tr>
            <td>May 2023</td>
            <td>Third yearly report on SCG recommendations to the 1+MG national mirror groups to facilitate genome/ clinical data integration, specified to the 1+MG Use Cases.</td>
            <td>D1.5</td>
            <td>WP1</td>
          </tr>
          <tr>
            <td>September 2022</td>
            <td>Report of ELSI Stakeholder Meetings</td>
            <td>D2.1</td>
            <td>WP2</td>
          </tr>
          <tr>
            <td>May 2023</td>
            <td>Policy document for a genome data sharing initiative</td>
            <td>D2.2</td>
            <td>WP2</td>
          </tr>
          <tr>
            <td>May 2023</td>
            <td>Report on legal set-up including Data Protection Impact Assessment (DPIA)</td>
            <td>D2.3</td>
            <td>WP2</td>
          </tr>
          <tr>
            <td>May 2023</td>
            <td>Report on data access and governance framework</td>
            <td>D2.4</td>
            <td>WP2</td>
          </tr>
          <tr>
            <td>May 2021</td>
            <td>Quality metrics for sequencing</td>
            <td>D3.1</td>
            <td>WP3</td>
          </tr>
          <tr>
            <td>May 2022</td>
            <td>Best practices for Next Generation Sequencing</td>
            <td>D3.2</td>
            <td>WP3</td>
          </tr>
          <tr>
            <td>May 2023</td>
            <td>The B1MG data analysis challenge</td>
            <td>D3.3</td>
            <td>WP3</td>
          </tr>
          <tr>
            <td>May 2021</td>
            <td>Phenotypic and clinical metadata framework - 1v0</td>
            <td>D3.4</td>
            <td>WP3</td>
          </tr>
          <tr>
            <td>May 2022</td>
            <td>Phenotypic and clinical metadata framework - 2v0</td>
            <td>D3.5</td>
            <td>WP3</td>
          </tr>
          <tr>
            <td>May 2023</td>
            <td>Phenotypic and clinical metadata framework - 3v0</td>
            <td>D3.6</td>
            <td>WP3</td>
          </tr>
          <tr>
            <td>May 2021</td>
            <td>Documented best practices in sharing and linking phenotypic and genetic data - 1v0</td>
            <td>D3.7</td>
            <td>WP3</td>
          </tr>
          <tr>
            <td>May 2022</td>
            <td>Documented best practices in sharing and linking phenotypic and genetic data - 2v0</td>
            <td>D3.8</td>
            <td>WP3</td>
          </tr>
          <tr>
            <td>May 2021</td>
            <td>Secure cross-border data access roadmap - 1v0</td>
            <td>D4.1</td>
            <td>WP4</td>
          </tr>
          <tr>
            <td>May 2022</td>
            <td>Secure data access demonstrator</td>
            <td>D4.2</td>
            <td>WP4</td>
          </tr>
          <tr>
            <td>May 2023</td>
            <td>Secure cross-border data access roadmap - 2v0</td>
            <td>D4.3</td>
            <td>WP4</td>
          </tr>
          <tr>
            <td>May 2022</td>
            <td>B1MG maturity level model and country-specific alignment within the model</td>
            <td>D5.1</td>
            <td>WP5</td>
          </tr>
          <tr>
            <td>May 2023</td>
            <td>Roadmap and guidance tool for countries</td>
            <td>D5.2</td>
            <td>WP5</td>
          </tr>
          <tr>
            <td>May 2023</td>
            <td>Economic models methodology and case studies</td>
            <td>D5.3</td>
            <td>WP5</td>
          </tr>
          <tr>
            <td>September 2020</td>
            <td>Project Handbook 1v0</td>
            <td>D6.1</td>
            <td>WP6</td>
          </tr>
          <tr>
            <td>November 2020</td>
            <td>Project Data Management Plan (confidential)</td>
            <td>D6.2</td>
            <td>WP6</td>
          </tr>
          <tr>
            <td>November 2020</td>
            <td>Communication Strategy</td>
            <td>D6.3</td>
            <td>WP6</td>
          </tr>
          <tr>
            <td>September 2021</td>
            <td>Project Handbook 2v0</td>
            <td>D6.4</td>
            <td>WP6</td>
          </tr>
          <tr>
            <td>March 2023</td>
            <td>Project Handbook - 3v0 - final version and lessons learnt</td>
            <td>D6.5</td>
            <td>WP6</td>
          </tr>
          <tr>
            <td>May 2021</td>
            <td>Analysis and assessment of mirror groups</td>
            <td>D6.6</td>
            <td>WP6</td>
          </tr>
          <tr>
            <td>May 2022</td>
            <td>Guiding principles and best practices examples for mirror groups</td>
            <td>D6.7</td>
            <td>WP6</td>
          </tr>
          <tr>
            <td>September 2021</td>
            <td>Policy briefs - 1v0</td>
            <td>D6.8</td>
            <td>WP6</td>
          </tr>
          <tr>
            <td>March 2023</td>
            <td>Policy briefs - 2v0</td>
            <td>D6.9</td>
            <td>WP6</td>
          </tr>
          <tr>
            <td>March 2023</td>
            <td>Report on sustainability design and funding routes for the delivery of PM cross-border</td>
            <td>D6.10</td>
            <td>WP6</td>
          </tr>
        </tbody>
      </table> -->
      <table class="table is-bordered mt-5">
        <thead>
        <tr class="is-selected is-size-5">
          <th>Name</th>
          <th>Number</th>
          <th>WP</th>
        </tr>
      </thead>
      <tbody>
        <tr>
          <td colspan="3" class="has-background-white-ter"><strong class="is-size-5 has-text-grey">September 2020</strong></td>
        </tr>
        <tr>
            <td><a href="https://zenodo.org/record/5018457#.YgYuW-77RpQ">Project Handbook 1v0</a></td>
          <td>D6.1</td>
          <td>WP6</td>
        </tr>
        <tr>
          <td colspan="4" class="has-background-white-ter"><strong class="is-size-5 has-text-grey">October 2020</strong></td>
        </tr>
        <tr>
            <td><a href="https://zenodo.org/record/5786736#.YgYul-77RpQ">Document describing the operational organisation and processes for the Stakeholder Coordination Group (SCG)</a></td>
          <td>D1.1</td>
          <td>WP1</td>
        </tr>
        <tr>
          <td colspan="4" class="has-background-white-ter"><strong class="is-size-5 has-text-grey">November 2020</strong></td>
        </tr>
        <tr>
          <td>Project Data Management Plan (confidential)</td>
          <td>D6.2</td>
          <td>WP6</td>
        </tr>
        <tr>
            <td><a href="https://zenodo.org/record/5018436#.YgYvB-77RpQ">Communication Strategy</a></td>
          <td>D6.3</td>
          <td>WP6</td>
        </tr>
        <tr>
          <td colspan="4" class="has-background-white-ter"><strong class="is-size-5 has-text-grey">May 2021</strong></td>
        </tr>
        <tr>
          <td><a href="https://zenodo.org/record/4813451#.YK_dVpNKj0o">Generation of a stakeholder portal within the project webpage containing the information resulting from the activities of the SCG</a></td>
          <td>D1.2</td>
          <td>WP1</td>
        </tr>
        <tr>
          <td>First yearly report on SCG recommendations to the 1+MG national mirror groups to facilitate genome/ clinical data integration, specified to the 1+MG Use Cases.</td>
          <td>D1.3</td>
          <td>WP1</td>
        </tr>
        <tr>
          <td><a href="https://zenodo.org/record/4889391#.YLYFWzZKhQY">Quality metrics for sequencing</a></td>
          <td>D3.1</td>
          <td>WP3</td>
        </tr>
        <tr>
          <td>Phenotypic and clinical metadata framework - 1v0</td>
          <td>D3.4</td>
          <td>WP3</td>
        </tr>
        <tr>
          <td><a href="https://zenodo.org/record/4819149#.YK_bKZNKj0o">Documented best practices in sharing and linking phenotypic and genetic data - 1v0</a></td>
          <td>D3.7</td>
          <td>WP3</td>
        </tr>
        <tr>
          <td><a href="https://zenodo.org/record/4889401#.YLYHHzZKhQY">Secure cross-border data access roadmap - 1v0</a></td>
          <td>D4.1</td>
          <td>WP4</td>
        </tr>
        <tr>
          <td><a href="https://zenodo.org/record/4813544#.YK_dbZNKj0o">Analysis and assessment of mirror groups</a></td>
          <td>D6.6</td>
          <td>WP6</td>
        </tr>
        <tr>
          <td colspan="4" class="has-background-white-ter"><strong class="is-size-5 has-text-grey">September 2021</strong></td>
        </tr>
        <tr>
            <td><a href="https://zenodo.org/record/5704929#.YgYvgO77RpQ">Project Handbook 2v0</a></td>
          <td>D6.4</td>
          <td>WP6</td>
        </tr>
        <tr>
            <td><a href="https://zenodo.org/record/5727650#.YgYvk-77RpQ">Policy briefs - 1v0</a></td>
          <td>D6.8</td>
          <td>WP6</td>
        </tr>
        <tr>
          <td colspan="4" class="has-background-white-ter"><strong class="is-size-5 has-text-grey">May 2022</strong></td>
        </tr>
        <tr>
          <td>Second yearly report on SCG recommendations to the 1+MG national mirror groups to facilitate genome/ clinical data integration, specified to the 1+MG Use Cases.</td>
          <td>D1.4</td>
          <td>WP1</td>
        </tr>
        <tr>
          <td>Best practices for Next Generation Sequencing</td>
          <td>D3.2</td>
          <td>WP3</td>
        </tr>
        <tr>
          <td>Phenotypic and clinical metadata framework - 2v0</td>
          <td>D3.5</td>
          <td>WP3</td>
        </tr>
        <tr>
          <td>Documented best practices in sharing and linking phenotypic and genetic data - 2v0</td>
          <td>D3.8</td>
          <td>WP3</td>
        </tr>
        <tr>
          <td>Secure data access demonstrator</td>
          <td>D4.2</td>
          <td>WP4</td>
        </tr>
        <tr>
          <td>B1MG maturity level model and country-specific alignment within the model</td>
          <td>D5.1</td>
          <td>WP5</td>
        </tr>
        <tr>
          <td>Guiding principles and best practices examples for mirror groups</td>
          <td>D6.7</td>
          <td>WP6</td>
        </tr>
        <tr>
          <td colspan="4" class="has-background-white-ter"><strong class="is-size-5 has-text-grey">September 2022</strong></td>
        </tr>
        <tr>
          <td>Report of ELSI Stakeholder Meetings</td>
          <td>D2.1</td>
          <td>WP2</td>
        </tr>
        <tr>
          <td colspan="4" class="has-background-white-ter"><strong class="is-size-5 has-text-grey">March 2023</strong></td>
        </tr>
        <tr>
          <td>Policy briefs - 2v0</td>
          <td>D6.9</td>
          <td>WP6</td>
        </tr>
        <tr>
          <td>Report on sustainability design and funding routes for the delivery of PM cross-border</td>
          <td>D6.10</td>
          <td>WP6</td>
        </tr>
        <tr>
          <td>Project Handbook - 3v0 - final version and lessons learnt</td>
          <td>D6.5</td>
          <td>WP6</td>
        </tr>
        <tr>
          <td colspan="4" class="has-background-white-ter"><strong class="is-size-5 has-text-grey">May 2023</strong></td>
        </tr>
        <tr>
          <td>Third yearly report on SCG recommendations to the 1+MG national mirror groups to facilitate genome/ clinical data integration, specified to the 1+MG Use Cases.</td>
          <td>D1.5</td>
          <td>WP1</td>
        </tr>
        <tr>
          <td>Policy document for a genome data sharing initiative</td>
          <td>D2.2</td>
          <td>WP2</td>
        </tr>
        <tr>
          <td>Report on legal set-up including Data Protection Impact Assessment (DPIA)</td>
          <td>D2.3</td>
          <td>WP2</td>
        </tr>
        <tr>
          <td>Report on data access and governance framework</td>
          <td>D2.4</td>
          <td>WP2</td>
        </tr>
        <tr>
          <td>The B1MG data analysis challenge</td>
          <td>D3.3</td>
          <td>WP3</td>
        </tr>
        <tr>
          <td>Phenotypic and clinical metadata framework - 3v0</td>
          <td>D3.6</td>
          <td>WP3</td>
        </tr>
        <tr>
          <td>Secure cross-border data access roadmap - 2v0</td>
          <td>D4.3</td>
          <td>WP4</td>
        </tr>
        <tr>
          <td>Roadmap and guidance tool for countries</td>
          <td>D5.2</td>
          <td>WP5</td>
        </tr>
        <tr>
          <td>Economic models methodology and case studies</td>
          <td>D5.3</td>
          <td>WP5</td>
        </tr>
      </tbody>
    </table>




        <h2 class="is-size-3 mb-2">See also:</h2>
        <ul class="mt-0">
          <li><a href="/about/how-organised">How the project is organised</a></li>
            <li><a href="/about/how-organised">Work Packages</a></li>
            <li><a href="/about/objectives">Objectives and key results</a></li>
        </ul>
    </div>
</section>
<?php
   include $_SERVER['DOCUMENT_ROOT']."/includes/footer.inc";
?>
